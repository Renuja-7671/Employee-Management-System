...
export async function POST(
  ...
  try {
    ...
    // Only Managing Director and RESERVED can approve leaves
    if (admin.adminType !== 'MANAGING_DIRECTOR' && admin.adminType !== 'RESERVED') {
      return NextResponse.json(
        { error: 'Only Managing Director or RESERVED admin can approve leave requests' },
        { status: 403 }
      );
    }
    ...
    // NOTE: Balance deduction now happens at leave application time (not at approval)
    // This allows employees to see real-time leave balance updates
